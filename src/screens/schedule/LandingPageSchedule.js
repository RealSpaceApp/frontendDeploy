...
const LandingPageSchedule = () => {
  ...
  const fetchUpcomingEvents = async () => {
    try {
      const response = await fetch('http://localhost:8080/event/schedule/upcoming-event-dates', {
        method: 'GET',
        headers: {
  ...
  const fetchEventsForDate = async (date) => {
    try {
      const response = await fetch(`http://localhost:8080/event/schedule/${date}T00:00:00Z`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
