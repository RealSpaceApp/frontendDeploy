...
const AddSchedule = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8080/call-schedule/get-schedules/${user_id}`, {
          headers: {
            Cookie: cookie || '',
          },
        });
  ...
  const handleSubmit = async () => {
    try {
      ...
      let response;
      if (scheduleId) {
        response = await axios.post(`http://localhost:8080/call-schedule/update/${scheduleId}`, body, {
          headers: {
        ...
        response = await axios.post('http://localhost:8080/call-schedule/create', body, {
          headers: {
            'Content-Type': 'application/json',
            Cookie: cookie || '',
          },
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await axios.post(`http://localhost:8080/call-schedule/remove/${scheduleId}`, {}, {
        headers: {
          'Content-Type': 'application/json',
          Cookie: cookie || '',
        },
